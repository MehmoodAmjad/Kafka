...
# Add Ranger Kafka Plugin
ENV RANGER_PLUGIN_HOME /opt/kafka/ranger-kafka-plugin
COPY ranger-${RANGER_VERSION}-kafka-plugin.tar.gz /opt//kafka_${SCALA_VERSION}-${KAFKA_VERSION}/ranger-${RANGER_VERSION}-kafka-plugin.tar.gz

WORKDIR /opt/kafka

RUN tar -xvf ranger-${RANGER_VERSION}-kafka-plugin.tar.gz \
    && ln -s ranger-${RANGER_VERSION}-kafka-plugin $RANGER_PLUGIN_HOME \
    && rm -f ranger-${RANGER_VERSION}-kafka-plugin.tar.gz
